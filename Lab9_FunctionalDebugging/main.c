...
int main(void){
    unsigned long i, P4, P0, P4old, P0old, statechanged;
    ...
    P4 = !((GPIO_PORTF_DATA_R & 0x10) >> 4);
    P0 = !(GPIO_PORTF_DATA_R & 0x01);
    P4old = P4;
    P0old = P0;
    ...
    while(1)
    {
        P4 = !((GPIO_PORTF_DATA_R & 0x10) >> 4); // if this is 1 then button PF4 not pressed
        if (P4old != P4)
        {
            statechanged = 1;
        }
        ...
        P0 = !(GPIO_PORTF_DATA_R & 0x01); // if this is 1 then button PF0 not pressed
        if (P0old != P0)
        {
            statechanged = 1;
        }
        ...
        if ( (P0 & 0x01) || (P4 & 0x01) ) // pf4 is 0 or pf0 is 0
        {        
        ...
        if ( !(P0 & 0x01) && !(P4 & 0x01) ) // pf4 is 1 and pf0 is 1, which means neither pressed
        {        
            GPIO_PORTF_DATA_R = GPIO_PORTF_DATA_R & 0xFD;
        }

        if (i < 50 && statechanged == 1)
        {
